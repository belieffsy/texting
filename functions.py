...
def check_events(settings,chequer,pieces,button_d2,button_r2,msg_box_reset):
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            elif settings.box_reset == True or settings.get_win==(1 or -1 or 0):
                if msg_box_reset.msg3_image_rect.collidepoint(x,y) or  msg_box_reset.circle_rect.collidepoint(x,y):
                    settings.box_reset = False
                    settings.start = True
...
def draw(settings,msg_box_reset,box_black_win,box_white_win,box_get_draw):
    #画消息框
...
def draw_box_win(settings,box_black_win,box_white_win):
    if settings.get_win == 1:
        x, y = pygame.mouse.get_pos()
